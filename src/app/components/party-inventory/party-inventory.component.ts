...
import { withLatestFrom, take, map, switchMap } from 'rxjs/operators';
...
export class PartyInventoryComponent implements OnDestroy {
   ...
   */
  currentIndex$: Observable<number> = combineLatest(
    this.doChangeIndex$,
    this.party$
  ).pipe(
    map(([index, party]) => {
  ...
  currentEntity$: Observable<EntityWithEquipment> = combineLatest(
    this.currentIndex$,
    this.party$
  ).pipe(
    map(combined => {
      const [index, party] = combined;
