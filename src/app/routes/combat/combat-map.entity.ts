...
export class CombatMapComponent extends GameTileMap
  ...
  enemies$: Observable<List<Combatant>> = this.store.select(
    getCombatEncounterEnemies
  );

  /** Observable<Entity[]> of player-card members */
  party$: Observable<List<Combatant>> = this.store.select(
    getCombatEncounterParty
  );
  ...
  renderFrame(view: TileMapView, elapsed: number) {
    ...
    this.party.forEach((component: CombatPlayerComponent) => {
      ...
      const sprites = component.findBehaviors(
        SpriteComponent
      ) as SpriteComponent[];
