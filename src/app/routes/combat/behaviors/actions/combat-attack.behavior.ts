...
export class CombatAttackBehaviorComponent extends CombatActionBehavior {
  ...
  act(then?: IPlayerActionCallback): boolean {
    ...
    const attack = () => {
      combineLatest(
        this.store.select(getEntityEquipment(attacker.model.eid)),
        this.store.select(getEntityEquipment(defender.model.eid))
      )
        .pipe(
          map(([equippedAttacker, equippedDefender]) => {
            const damage: number = this.combatService.attackCombatant(
