 ...
 */
export function storeItemsFilter<T extends ITemplateBaseItem>(
  items: Immutable.List<T>,
  groups: string[],
  level: number
): Immutable.List<T> {
  return items
    .filter((i: ITemplateBaseItem) => {
      const levelMatch: boolean =
        typeof i.level === 'undefined' || i.level === level;
 ...
 * @returns {ITemplateBaseItem[]} The filtered item list
 */
export function sellItemsFilter<T extends ITemplateBaseItem>(
  items: Immutable.List<T>,
  groups: string[]
) {
  return items
...
export class StoreFeatureComponent extends TiledFeatureComponent
   ...
   */
  partyInventory$: Observable<Immutable.List<Item>> = combineLatest(
    combineLatest(this.store.select(getGameInventory), this.category$).pipe(
      map(([inventory, category]) => {
        return inventory.filter(i => i && i.category === category).toList();
      })
    ),
    this.groups$
  ).pipe(
    map(([items, groups]) => {
      return sellItemsFilter(items, groups);
    })
  );
  ...
  inventory$: Observable<Immutable.List<ITemplateBaseItem>> = combineLatest(
    ...
    map(([items, groups, level]) => {
      return storeItemsFilter(items.toList(), groups, level);
    })
  );
  ...
  isBuying$: Observable<boolean> = combineLatest(
    this.selected$,
    this.selling$
  ).pipe(
    map(([selected, selling]) => {
      return !selected && !selling;
    })
  );

  isSelling$: Observable<boolean> = combineLatest(
    this.selected$,
    this.selling$
  ).pipe(
    map(([selected, selling]) => {
    ...
    .pipe(
      withLatestFrom(
        ...
        (evt, model: GameState) => {
          ...
          if (isSelling) {
            this.notify.show(
              `Sold ${item.name} for ${item.value} gold.`,
              null,
              1500
            );
