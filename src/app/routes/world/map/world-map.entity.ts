...
export class WorldMapComponent extends GameTileMap
  ...
  @ViewChildren(MapFeatureComponent) mapFeatures: QueryList<
    MapFeatureComponent
  >;
  ...
  resource$: Observable<
    TiledTMXResource
  > = this.gameStateService.worldMap$.pipe(
    ...
    map(([keyMap, features]) => {
      return features.filter((f: TiledMapFeatureData) => {
        ...
        return (
          !f.properties ||
          !f.properties.id ||
          keyMap.get(f.properties.id) !== true
        );
      });
    })
  );
  ...
  renderPoint$: Observable<Point> = combineLatest(
    this.store.select(getGamePartyPosition),
    this._renderPoint$
  ).pipe(
